...
spec = describe "Houseman" $ do
  describe "runProcess" $ do
    it "should run given process" $ do
      log' <- newChan
      _ <- capture . waitForProcess =<< Houseman.runProcess (Proc "echo" "./test/fixtures/echo.sh" ["foo", "🙈"] [("ECHO", "1")]) log'
    ...
    it "should use .env as dotenv file, which supersedes original environment" $ inTempDirectory $ do
      ...
      writeFile ".env" "BAZ=2"
      log' <- newChan
      _ <- capture . waitForProcess =<< Houseman.runProcess (Proc "echo" "printenv" ["BAZ"] []) log'
